...
import {
  ...
  decreaseQuantity,
  increaseQuantity,
} from "../redux/atelieSlice";
import { FaMinus, FaPlus } from "react-icons/fa";
import { cn } from "./ui/cn";

const AddToCartButton = ({ item, className }) => {
  const dispatch = useDispatch();
  const { products } = useSelector((state) => state.atelieReducer);
    ...
    <>
      {existingProduct ? (
        ...
        <button
          ...
          className="w-full border border-black text-black text-xs font-medium py-3 px-6 uppercase tracking-wide hover:bg-black hover:text-white transition-all duration-200 rounded-xl"
        >
          Add to cart
        </button>
      )}
